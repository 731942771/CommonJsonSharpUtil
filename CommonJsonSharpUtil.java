 ...
 * @author cuiweiyou
 */
public class CommonJsonSharpUtil {
	...
	/**
	 * 动态创建Bean<br/>
	 * 仅须固定格式的json数据，不再需要对应创建bean的java文件。有cglib在内存完成<br/>
	 * 尚未适配List/Map等数据类型的成员变量
	 * @param json 字串
	 * @throws ClassNotFoundException 反射时可能出现的异常
	 */
	public Object analyze2Object(String json){
		...
		try {
			...
			JSONObject jsonObject = jobj.getJSONObject(beanName);
			...
			while(keys2.hasNext()){
				...
				propertyMap.put(key, Class.forName(value.getClass().getName()));
			}
			
			// 2.生成Bean对象
			BeanFactory bean = new BeanFactory(propertyMap);
			
			keys2 = jsonObject.keys();
			while(keys2.hasNext()){
				String key = keys2.next();
				Object value = jsonObject.get(key);
				bean.setValue(key, value); // id = 123;
			}
